#!/usr/bin/env python3
"""Run 3 sequential test pipelines v2 (after quality gate fixes) and save outputs."""
import requests, json, time, sys

BASE = "http://localhost:3002/api"
GOAL = "reverse aging in the human brain"

def run_pipeline(run_num):
    print(f"\n{'='*70}")
    print(f"RUN {run_num}/3")
    print(f"{'='*70}")
    payload = {
        "goal": GOAL,
        "test_mode": True,
    }
    r = requests.post(f"{BASE}/run-full-pipeline", json=payload)
    r.raise_for_status()
    run_id = r.json().get("run_id")
    print(f"Run ID: {run_id}")

    start = time.time()
    while time.time() - start < 600:
        r = requests.get(f"{BASE}/full-pipeline-result", params={"run_id": run_id})
        elapsed = int(time.time() - start)
        if r.status_code == 200:
            result = r.json()
            fname = f"genius-v2-run{run_num}-{int(time.time())}.json"
            with open(fname, "w") as f:
                json.dump(result, f, indent=2)
            print(f"\nDONE in {elapsed}s — saved to {fname}")
            return fname, result
        elif r.status_code == 202:
            p = r.json()
            print(f"  [{elapsed}s] Step {p.get('step','?')}: {p.get('detail','')}          ", end="\r")
        time.sleep(5)
    print("\nTIMEOUT!")
    return None, None

results = []
for i in range(1, 4):
    fname, result = run_pipeline(i)
    if result and result.get("success"):
        results.append((fname, result))
    else:
        print(f"RUN {i} FAILED")
    time.sleep(2)

print(f"\n\n{'='*70}")
print(f"GENIUS V2 ANALYSIS ACROSS {len(results)} RUNS")
print(f"{'='*70}")

for idx, (fname, result) in enumerate(results):
    so = result.get("step_outputs", {})
    summary = result.get("summary", {})
    step9 = so.get("step9", {})
    l6s = step9.get("l6_tasks", [])
    l5s = step9.get("l5_nodes", [])
    step7 = so.get("step7", {})
    ihs = step7.get("instantiation_hypotheses", [])

    print(f"\n--- RUN {idx+1} ({fname}) ---")
    print(f"Goals: {summary.get('goals')}, S-nodes: {summary.get('total_s_nodes')}, L3: {summary.get('total_l3_questions')}, IHs: {summary.get('total_ihs')}, L4: {summary.get('total_l4_questions')}, L5: {summary.get('total_l5_nodes')}, L6: {summary.get('total_l6_tasks')}")

    # if_null coverage
    has_if_null = sum(1 for t in l6s if t.get("if_null"))
    print(f"  if_null: {has_if_null}/{len(l6s)}")

    # Feasibility
    feas = [t.get("feasibility_score") for t in l6s if t.get("feasibility_score") is not None]
    avg_f = sum(feas)/len(feas) if feas else 0
    print(f"  Avg feasibility: {avg_f:.1f}, Range: {min(feas) if feas else 0}-{max(feas) if feas else 0}")

    # Discovery
    disc_exp = sum(1 for t in l6s if t.get("discovery_component"))
    print(f"  Discovery experiments: {disc_exp}/{len(l6s)}")

    # System diversity
    systems_used = {}
    for t in l6s:
        s = str(t.get("simt_parameters", {}).get("system", "")).lower()
        for sys_name in ["C57BL/6J", "Drosophila", "C. elegans", "zebrafish", "organoid", "iPSC", "computational", "mathematical", "ODE", "agent-based", "cell-free", "reconstituted", "yeast", "human", "marmoset", "macaque", "ex vivo"]:
            if sys_name.lower() in s:
                systems_used[sys_name] = systems_used.get(sys_name, 0) + 1
    print(f"  Systems: {systems_used}")

    # Check anti-patterns
    omics_default = 0
    comp_filler = 0
    eg_hedging = 0
    aged_vs_young_only = 0
    for t in l6s:
        simt = t.get("simt_parameters", {})
        s_text = str(simt.get("system", "")).lower()
        i_text = str(simt.get("intervention", "")).lower()
        m_text = str(simt.get("meter", "")).lower()
        title = str(t.get("title", "")).lower()

        # Omics default: RNA-seq/proteomics without perturbation
        if ("rna-seq" in m_text or "proteomics" in m_text or "proteome" in m_text or "transcriptom" in m_text) and ("no" in i_text[:20] or "passive" in i_text or "no direct" in i_text):
            omics_default += 1

        # Computational filler
        if ("computational" in s_text or "agent-based" in s_text or "ode" in s_text or "simulation" in s_text or "model" in title[:30]) and ("sensitivity" in title or "parameter" in title or "calibrat" in title):
            comp_filler += 1

        # e.g. hedging in SIMT
        for field in [simt.get("system", ""), simt.get("intervention", ""), simt.get("meter", "")]:
            if "e.g." in str(field) or "e.g.," in str(field):
                eg_hedging += 1
                break

        # Aged vs young only
        if ("no" in i_text[:20] or "no direct" in i_text or "passive" in i_text) and ("aged" in s_text or "old" in s_text):
            aged_vs_young_only += 1

    print(f"  ANTI-PATTERNS: omics_default={omics_default}, comp_filler={comp_filler}, eg_hedging={eg_hedging}, aged_vs_young={aged_vs_young_only}")

    # Competition matrix
    comp = step7.get("competition_matrix", [])
    print(f"  Competition matrix: {len(comp)} entries")

    # Print 5 L6 experiments
    print(f"\n  TOP 5 L6 EXPERIMENTS:")
    for t in l6s[:5]:
        simt = t.get("simt_parameters", {})
        print(f"    {t.get('id')}: {t.get('title','')[:140]}")
        rat = t.get('rationale', '')
        if rat:
            print(f"      WHY: {str(rat)[:200]}")
        if_null = t.get('if_null', '')
        if if_null:
            print(f"      IF NULL: {str(if_null)[:200]}")
        print(f"      S: {str(simt.get('system',''))[:100]}")
        print(f"      I: {str(simt.get('intervention',''))[:100]}")
        print(f"      M: {str(simt.get('meter',''))[:100]}")
        print(f"      Feas: {t.get('feasibility_score')}")
        print()

print(f"\nAll output files: {[r[0] for r in results]}")
